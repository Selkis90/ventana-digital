...
function mostrarVideoRemoto(stream, peerId) {
    ...
    if (peerIdRemoto === peerId && videoRemoto.srcObject === stream && !videoRemoto.paused) {
        console.log('ℹ️ Video ya está reproduciendo correctamente');
        return;
    }
    
    // 🔥 SI EL STREAM YA ESTÁ ASIGNADO PERO ESTÁ PAUSADO, SOLO REANUDAR
    if (videoRemoto.srcObject === stream && videoRemoto.paused) {
        console.log('ℹ️ Video pausado, reanudando...');
    ...
    // 🔥 SOLO LIMPIAR SI ES UN STREAM DIFERENTE
    ...
    // 🔥 REPRODUCIR CON REINTENTO CONTROLADO
    let playAttempts = 0;
    const maxPlayAttempts = 3;
    
    const playMedia = () => {
        if (playAttempts >= maxPlayAttempts) {
            console.warn(`⚠️ Máximo de intentos de reproducción alcanzado`);
            isConnecting = false;
            return;
        }
        playAttempts++;
        ...
        try {
            videoRemoto.play()
                ...
                .catch(err => {
                    console.warn(`⚠️ Error al reproducir video (intento ${playAttempts}/${maxPlayAttempts}): ${err.message}`);
                    if (playAttempts < maxPlayAttempts) {
                        setTimeout(playMedia, 500);
                    } else {
                        console.error(`❌ No se pudo reproducir el video después de ${maxPlayAttempts} intentos`);
                        isConnecting = false;
                    }
                });
        } catch (error) {
...
function iniciarOferta(targetId, pc) {
    ...
    // 🔥 VERIFICAR ESTADO DE SIGNALING
    if (pc.signalingState !== 'stable') {
        console.log(`⚠️ Signaling state no es stable: ${pc.signalingState}, esperando...`);
        setTimeout(() => {
            if (pc.signalingState === 'stable' && peers[targetId]) {
                iniciarOferta(targetId, pc);
            } else {
                isConnecting = false;
                console.warn(`⚠️ Timeout esperando state stable para ${targetId}`);
            }
        }, 1000);
        return;
    }
...
function conectarConTodos(clientes) {
    ...
    if (peers[targetId]) {
        ...
        console.log(`ℹ️ Peer existente: connectionState=${state}, signalingState=${signalingState}`);
        ...
        if (signalingState === 'have-local-offer' || signalingState === 'have-remote-offer') {
            console.log(`⏳ En negociación con ${targetId}, esperando...`);
            return;
        }
...
socket.on("offer", async (data) => {
    ...
    // 🔥 VERIFICAR ESTADO DE SIGNALING
    const existingPc = peers[from];
    if (existingPc) {
        const state = existingPc.signalingState;
        console.log(`ℹ️ Signaling state actual: ${state}`);
        
        if (state === 'have-local-offer' || state === 'have-remote-offer') {
            console.log(`⏳ Ya en negociación con ${from}, ignorando oferta`);
            return;
        }
    ...
    // 🔥 LIMPIAR ANTES DE CREAR NUEVA CONEXIÓN
    limpiarPeer(from);
...
socket.on("answer", async (data) => {
    ...
    console.log(`📊 Signaling state actual: ${currentState}`);
    ...
    if (currentState === 'have-remote-offer') {
        console.log(`⚠️ Estado have-remote-offer, ignorando respuesta`);
        return;
    }

    if (currentState !== 'have-local-offer') {
        console.log(`⚠️ Estado incorrecto: ${currentState} para ${from}`);
        limpiarPeer(from);
        isConnecting = false;
