...
let isConnecting = false; // EVITA MÚLTIPLES CONEXIONES SIMULTÁNEAS
...
function mostrarVideoRemoto(stream, peerId) {
    ...
    // Si el video remoto ya tiene el mismo stream, no hacer nada
...
function crearPeerConnection(targetId, iceRestart = false) {
    ...
    pc.oniceconnectionstatechange = () => {
        ...
        if (pc.iceConnectionState === "disconnected") {
            console.log(`🔴 ICE desconectado con ${targetId}, intentando reconectar...`);
        }
    };
    ...
    pc.onnegotiationneeded = () => {
        ...
        // No hacer nada aquí, la negociación se maneja manualmente
    };
...
function conectarConTodos(clientes) {
    ...
    // Solo conectar con el primer cliente disponible (evita múltiples conexiones)
    ...
    if (peers[targetId]) {
        ...
        // Si está en otro estado, limpiar y recrear
        limpiarPeer(targetId);
    }
...
socket.on("offer", async (data) => {
    ...
    console.log(`🔄 ICE restart en oferta: ${offer.type === 'offer' ? 'false' : 'true'}`);
